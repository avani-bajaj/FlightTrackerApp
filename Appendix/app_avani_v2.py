...
def adjust_zoom(delta: float):
    st.session_state.ui_zoom = clamp_zoom(st.session_state.get("ui_zoom", 1.0) + delta)
...
def build_css(dark_mode: bool, zoom_scale: float) -> str:
    palettes = {
        True: {
            "bg": "#03030a",
            "panel": "#090d1a",
    ...
    zoom_clamped = clamp_zoom(zoom_scale)

    return f"""
    <style>
        :root {{
            --zoom-scale: {zoom_clamped:.2f};
        }}
        {button_color_fix}
        html, body {{
        ...
        .hero-title {{
            font-size: calc(2.2rem * var(--zoom-scale));
            font-weight: 700;
        ...
        .chat-bubble {{
            ...
            border-radius: 20px;
            line-height: 1.45;
            font-size: calc(0.98rem * var(--zoom-scale));
            max-width: 85%;
            margin: 0.9rem 0;
        }}
        ...
        .chat-input .stButton button {{
            width: calc(54px * var(--zoom-scale));
            height: calc(54px * var(--zoom-scale));
            border-radius: 999px;
            border: none;
            font-size: calc(1.45rem * var(--zoom-scale));
...
st.markdown(
    build_css(st.session_state.dark_mode, st.session_state.ui_zoom),
...
with top_cols[1]:
    ...
    if toggle_value != st.session_state.dark_mode:
        ...
        st.rerun()
    zoom_cols = st.columns([1, 1, 1])
    zoom_out_disabled = st.session_state.ui_zoom <= ZOOM_MIN + 1e-6
    zoom_in_disabled = st.session_state.ui_zoom >= ZOOM_MAX - 1e-6
    with zoom_cols[0]:
        if st.button("➖", key="zoom_out_btn", help="Zoom out", disabled=zoom_out_disabled):
            adjust_zoom(-ZOOM_STEP)
            st.rerun()
    with zoom_cols[1]:
        st.markdown(
            f'<div class="zoom-level">{int(st.session_state.ui_zoom * 100)}%</div>',
            unsafe_allow_html=True,
        )
    with zoom_cols[2]:
        if st.button("➕", key="zoom_in_btn", help="Zoom in", disabled=zoom_in_disabled):
            adjust_zoom(ZOOM_STEP)
            st.rerun()
st.markdown('</div>', unsafe_allow_html=True)
